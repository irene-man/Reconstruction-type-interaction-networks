import numpy as np
import pandas as pd
from ode_model_assortative import get_steady_state
...
def simulate_parameters(df, n_types, p_intra, c, analysis):
    ...
    for j in range(n_sets):
        ...
        if n_success_sets < n_required_success_sets:
            ...
            this_eq_l, this_eq_h = np.array_split(this_eq, 2)
            marginals = marginal_operator.dot(this_eq_l + this_eq_h)
            this_eq_l = this_eq_l / sum(this_eq_l)
            this_eq_h = this_eq_h / sum(this_eq_h)
            eps = 0.001
            if min(marginals) > eps:
                ...
                for num_objects in [100, 1000, 10000, 100000]:
                    ...
                    # output = np.zeros((num_objects, n_types + 1), dtype=int)
                    # for k in range(len(states) * 2):
                    #     start = np.sum(realisations[:k])
                    #     end = np.sum(realisations[:(k+1)])
                    #     state = np.zeros(n_types)
                    #     state_id = k % len(states)
                    #     if state_id != 0:
                    #         state[np.array(states[state_id]._types) - 1] = 1
                    #     output[start:end, :n_types] = state
                    #     if k >= len(states):
                    #         output[start:end, -1] = 1
                    # Sample objects
                    realisations_l = np.random.multinomial(num_objects, this_eq_l)
                    realisations_h = np.random.multinomial(num_objects, this_eq_h)
                    output_l = np.zeros((num_objects, n_types), dtype=int)
                    output_h = np.zeros((num_objects, n_types), dtype=int)
                    for k in range(1, len(states)):
                        start_l = np.sum(realisations_l[:k])
                        start_h = np.sum(realisations_h[:k])
                        end_l = np.sum(realisations_l[:(k+1)])
                        end_h = np.sum(realisations_h[:(k+1)])
                        state = np.zeros(n_types)
                        state[np.array(states[k]._types) - 1] = 1
                        output_l[start_l:end_l] = state
                        output_h[start_h:end_h] = state

                    # Write the realisations in a csv file
                    col_names = ['x_{}'.format(i + 1) for i in range(n_types)]
                    output_df_l = pd.DataFrame(output_l, columns = col_names)
                    output_df_h = pd.DataFrame(output_h, columns = col_names)
                    folder = '/home/iman/Research/Graphical/simulation/data_equilibria/{}/'.format(analysis)
                    filename_l = '{}_equilibrium_nTypes{}_k{}_h{}_pIntra{}_c{}_nObjects{}_set{}_l'.format(analysis, n_types, k_bound, h_bound, p_intra, c, num_objects, n_success_sets).replace(".", "_") + '.csv'
                    filename_h = '{}_equilibrium_nTypes{}_k{}_h{}_pIntra{}_c{}_nObjects{}_set{}_h'.format(analysis, n_types, k_bound, h_bound, p_intra, c, num_objects, n_success_sets).replace(".", "_") + '.csv'
                    output_df_l.to_csv(folder + filename_l)
                    output_df_h.to_csv(folder + filename_h)
...
if __name__ == '__main__':
    ...
    p_intra = 0.25
    c = 3
    analysis = 'basecase_subpopulation'
